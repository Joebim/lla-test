...
const QuestPreview = ({ params }: { params: { id: string } }) => {
    ...
    async function fetchPreviewQuest() {
      ...
      try {
        ...
        toast({
          title: "Error",
          description:
            "error occured while fetching quests for this particular user",
          variant: "critical",
        });
      }
    }
  ...
  const currentThumbnail = questPreview?.previews[indexNo]?.thumbnail;
    ...
    <section className="flex min-h-screen flex-col gap-6 bg-neutral-5 px-[20px] pb-[74px] pt-[50px] font-axiforma sm:px-[74px]">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center lg:gap-8">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-wrap gap-[10px] transition-all sm:flex-nowrap sm:gap-4">
            {questPreview &&
                ...
                >
                  {quest.thumbnail && (
                    <Image
                      ...
                      className={`${indexNo === index && "rounded-[10px] border-2 border-solid border-[orange]"} size-[120px] h-[70px] w-[70px] cursor-pointer rounded-[10px] duration-200 hover:scale-105 sm:h-[100px] sm:w-[100px] sm:rounded-2xl`}
                      placeholder="blur"
      ...
      {isLoading ? (
        <Skeleton className="h-screen w-full bg-neutral-70" />
      ) : (
        <div
          className="flex h-[731px] w-full items-end self-stretch rounded-3xl bg-cover bg-no-repeat p-5 sm:p-10 lg:p-20"
          style={{
            backgroundImage: `url('${currentThumbnail || "/quests-preview/burning-building.png"}')`,
            ...
            <div className="flex items-end justify-end gap-6">
              ...
              <button
                ...
                onClick={() => {
                  if (
                    questPreview &&
                    indexNo < questPreview?.previews.length - 1
                  ) {
